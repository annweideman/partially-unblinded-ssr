...
p0C_values<-0.2
p0T_values<-c(0.05, 0.1, 0.15, 0.175, 0.21, 0.225, 0.25, 0.3, 0.35)
#p0C_values <- 0.2
#p0T_values <- c(0.05, 0.088, 0.113, 0.139, 0.21, 0.235, 0.276, 0.327, 0.4)
...
for (p0T_val in p0T_values) {
  for (p0C_val in  p0C_values) {
    ...
    plot_list[[paste(p0T_val, p0C_val, sep = "_")]] <- 
      make_plot(data_subset, p0C_val=round(p0C_val,3),
                p0T_val=round(p0T_val,3), show_x_axis_label)
  }
}
