#-------------------------------------------------------------------------------
# Topic: Plots of type 1 error for output from script sims_t1_error_hetero.R
...
# Modify this URL according to the output you want to plot (hetero only for this script)
url<-"https://raw.githubusercontent.com/annweideman/partially-unblinded-ssr/main/output/results_p00.2_k0.5_seed12345_hetero.R"
load(url(url))
...
plot <- ggplot(results_long, aes(x = n0, y = type_1_error, 
  ...
  scale_y_continuous(limits = c(0.03, 0.06), breaks = seq(0.03, 0.06, by = 0.01)) + 
  theme(
